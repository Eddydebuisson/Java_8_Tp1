...
public class Stream_01_Test {
	...
	public void test_stream_anyMatch() throws Exception {
		...
		Boolean result1 = pizzas.stream().filter(pizza -> pizza.getPrice() >= 1300).findAny().isPresent();

		// TODO valider si au moins une pizza à un prix >= 2000
		Boolean result2 = pizzas.stream().filter(pizza -> pizza.getPrice() >= 2000).findAny().isPresent();
